...
import { Sheet, SheetContent, SheetTitle, SheetTrigger } from "@work/components/ui/sheet";
import { useAms } from "@work/lib/ams/store";
import { cn } from "@work/lib/utils";
import { useAuth } from "@/lib/auth";

const NAV = [
  { to: "/work-craft/work-orders", label: "Assembly Work Orders", icon: ClipboardList },
  { to: "/work-craft/rework-scrap", label: "Rework & Scrap", icon: Recycle },
  { to: "/work-craft/exceptions", label: "Assembly Exceptions", icon: ShieldAlert },
  { to: "/work-craft/finished-goods", label: "Finished Goods", icon: PackageCheck },
  { to: "/work-craft/certificates", label: "Completion Certificates", icon: Award },
] as const;
...
function Breadcrumbs() {
  ...
  return (
    <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1 text-xs text-muted-foreground">
      <Link to="/work-craft/work-orders" className="hover:text-foreground">
        Assembly Management
      </Link>
      {crumbs.map((c, i) => (
        <Fragment key={c.href}>
