...
export type User = { name: string; email: string; role: "Administrator" | "Data Steward" };
const users = [
  { name: "Nexus Administrator", email: "admin@nexuswms.com", id: "ADM-0001", password: "Admin@2026", role: "Administrator" as const },
...
function sharedSession(): User | null { try { const raw = document.cookie.split("; ").find((part) => part.startsWith("nexuswms_sso="))?.split("=").slice(1).join("="); if (!raw) return null; const value = JSON.parse(decodeURIComponent(raw)) as User; return value.role === "Administrator" || value.role === "Data Steward" ? value : null; } catch { return null; } }
