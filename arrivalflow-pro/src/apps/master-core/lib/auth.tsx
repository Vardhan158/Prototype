...
export type User = { name: string; email: string; role: "Warehouse Gate Entry & Arrival Management" | "Master Data Management" };
const users = [
  ...
  { name: "Amara Okafor", email: "amara@nexuswms.com", id: "DST-1001", password: "master@2026", role: "Master Data Management" as const },
];
const key = "nexuswms.master.session.v1";
function sharedSession(): User | null { try { const raw = document.cookie.split("; ").find((part) => part.startsWith("nexuswms_sso="))?.split("=").slice(1).join("="); if (!raw) return null; const value = JSON.parse(decodeURIComponent(raw)) as User; return value.role === "Warehouse Gate Entry & Arrival Management" || value.role === "Master Data Management" ? value : null; } catch { return null; } }
