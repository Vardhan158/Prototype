...
import { DocumentUploader } from "@/apps/master-core/integrated/components/DocumentUploader";
...
export function RecordFormSheet({
  ...
  const allSteps = useMemo(
    () => [...steps, { title: "Preview", description: "Review before submitting", fields: [] }],
    [steps],
  );
          ...
          <SheetHeader className="border-b border-border px-6 py-4">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <SheetDescription className="truncate">{currentStep.description}</SheetDescription>
              </div>
                    ...
                    <dl className="mt-3 grid gap-x-6 gap-y-3 sm:grid-cols-2">
                      {s.fields.map((f) => (
                        <div key={f.name} className="min-w-0">
                          ...
                          <dd className="truncate text-sm font-medium">{values[f.name] || "—"}</dd>
                        </div>
                      ))}
                    </dl>
                  </div>
