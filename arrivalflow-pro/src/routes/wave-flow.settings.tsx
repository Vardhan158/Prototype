...
import { ROLES, useRole } from "@/apps/wave-flow/integrated/context/role-context";
import { carriers, docks, warehouses, zones } from "@/apps/wave-flow/integrated/data/mock-data";
  ...
  head: () => ({
    meta: [
      { title: "Warehouse Settings | NEXUS WMS" },
      {
        name: "description",
        content:
          "Configure warehouse defaults, wave capacity rules, carrier options and role access for outbound fulfillment.",
      },
      { property: "og:title", content: "Warehouse Settings | NEXUS WMS" },
      {
        property: "og:description",
        content: "Operational configuration for the outbound fulfillment module.",
      },
    ],
  }),
          ...
          <Button
            onClick={() =>
              toast.success("Settings saved", {
                description: "TODO: persist via configuration service.",
              })
            }
          >
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <Card className="border-border shadow-[var(--shadow-card)]">
          <CardHeader>
            ...
            <CardDescription>
              Thresholds enforced during wave planning and execution.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
        ...
        <Card className="border-border shadow-[var(--shadow-card)]">
          ...
          <CardContent className="space-y-1">
            <Toggle
              label="Block wave release without confirmed reservation"
              hint="BR-150"
              defaultChecked
            />
            <Separator />
            <Toggle
              label="Require barcode verification on every pick"
              hint="BR-152"
              defaultChecked
            />
            <Separator />
            <Toggle
              label="Require load verification before dispatch"
              hint="BR-156"
              defaultChecked
            />
            <Separator />
            <Toggle
              label="Restrict dispatch approval to Warehouse Managers"
              hint="BR-157"
              defaultChecked
            />
            <Separator />
            <Toggle
              label="Auto-create backorders on short allocation"
              hint="BR-158"
              defaultChecked
            />
          </CardContent>
